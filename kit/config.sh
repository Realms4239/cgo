...
: "${CFG_SSH_PASSWORD:=}"; : "${CFG_VM_NAME:=}"; : "${CFG_VMX_PATH:=}"
: "${CFG_SNAPSHOT:=}"; : "${CFG_PROJECT_DIR:=/home/altfloat/cgo}"
: "${CFG_DASHBOARD_PORT:=9090}"; : "${CFG_GO_MIN_VERSION:=1.25}"
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  if [[ "$line" =~ ^([[:space:]]*)?([A-Za-z_]+):[[:space:]]*(.*)$ ]]; then
    ...
    case "$k" in
      ...
      vmx_path) CFG_VMX_PATH="$(_expand "$v")";; snapshot) CFG_SNAPSHOT="$v";;
...
export CFG_SSH_USER CFG_SSH_HOST CFG_SSH_PORT CFG_SSH_KEY CFG_SSH_PASSWORD \
       CFG_VM_NAME CFG_VMX_PATH CFG_SNAPSHOT CFG_PROJECT_DIR \
