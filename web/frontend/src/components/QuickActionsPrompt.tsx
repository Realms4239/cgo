...
export default function QuickActionsPrompt(){
  ...
  useEffect(()=>{
    ...
    const onKey=(e:KeyboardEvent)=>{ if(e.key==='Escape'){ clearTimeout(id); if(ref.current) animatePromptExit(ref.current).then(()=>setOpen(false)); else setOpen(false) } }
    el?.addEventListener('mouseenter',onEnter)
    el?.addEventListener('mouseleave',onLeave)
    document.addEventListener('keydown',onKey)
    return()=>{ clearTimeout(id); el?.removeEventListener('mouseenter',onEnter); el?.removeEventListener('mouseleave',onLeave); document.removeEventListener('keydown',onKey) }
  },[open])
  ...
  if(!open) return null
  return <div ref={ref} role="dialog" aria-label="Actions rapides" style={{position:'fixed', bottom:24, left:'50%', transform:'translateX(-50%)', display:'flex', gap:8, padding:'10px 14px', background:'rgba(16,16,18,0.92)', border:'1px solid rgba(255,255,255,0.08)', backdropFilter:'blur(12px)', borderRadius:10, zIndex:400, boxShadow:'0 8px 32px rgba(0,0,0,0.4)'}}>
